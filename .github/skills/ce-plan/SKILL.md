...
argument-hint: "[optional: feature description, `deepen [plan path]`, requirements doc path, or any task to plan]"
---
...
When this skill is used for **Acepe** software work, default to a **Deep** plan. Treat Acepe implementation planning as cross-cutting product work unless the request is clearly non-software. This is a **plan depth** override, not an instruction to trigger the separate "deepen an existing plan" fast path for an already-written plan unless the user explicitly asks for that, for example with `/ce:plan deepen` or `/ce:plan deepen docs/plans/<plan>.md`.
...
**Deepen intent:** The word "deepen" (or "deepening") in reference to a plan is the primary trigger for the deepening fast path. This includes explicit invocations like `/ce:plan deepen` and `/ce:plan deepen docs/plans/<plan>.md`. When the user says "deepen the plan", "deepen my plan", "run a deepening pass", or similar, the target document is a **plan** in `docs/plans/`, not a requirements document. Use any path, keyword, or context the user provides to identify the right plan. If a path is provided, verify it is actually a plan document. If the match is not obvious, confirm with the user before proceeding.
...
**Acepe override:** For software work in this repository, classify the plan as **Deep** unless the request is clearly for a non-software planning artifact.
